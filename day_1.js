...
function trySum(){
    if(input[indexes[0]] + input[indexes[1]] + input[indexes[2]] == target){
        console.log(`found it at ${input[indexes[0]]}, ${indexes[1]}, and ${input[indexes[2]]} which multiplied together equal ${input[indexes[0]] * input[indexes[1]] * input[indexes[2]]}`)
        return
    }

    if(++indexes[2] == input.length){
        if(++indexes[1] == input.length){
            console.log(indexes[0])
            if(++indexes[0] == input.length){
