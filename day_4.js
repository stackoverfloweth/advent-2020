...
input.forEach(passport => {
    //console.log(passport, isValid(passport))
    isValid2(passport) && count++
})

console.log(count)
console.log(validatePid({},'000000001'))
console.log(validatePid({},'0123456789'))
...
function isValid(passport) {
    ...
    return requiredField.every(field => passport[field] != undefined)
}
...
function isValid2(passport){
    ...
    const validations = {
        'ecl': validateEcl,'pid': validatePid,'eyr': validateEyr,'hcl': validateHcl,'byr': validateByr,'iyr': validateIyr,'hgt': validateHgt,//'cid'
    }

    return requiredField.every(field => passport[field] != undefined && validations[field](passport, passport[field]))
}

function validateEcl(passport, value){
...
function validatePid(passport, value){
...
function validateEyr(passport, value){
    //eyr (Expiration Year) - four digits; at least 2020 and at most 2030.

    var regex = new RegExp(/^\d{4}$/, "g")
    return regex.test(value) && Number(value) >= 2020 &&  Number(value) <= 2030
}

function validateHcl(passport, value){
    //hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.

    var regex = new RegExp(/^#[0-9a-fA-F]{6}$/, "g");
    return regex.test(value)
}

function validateByr(passport, value){
    //byr (Birth Year) - four digits; at least 1920 and at most 2002.

    var regex = new RegExp(/^\d{4}$/, "g");
    return regex.test(value) && Number(value) >= 1920 &&  Number(value) <= 2002
}
...
function validateHgt(passport, value){
    // hgt (Height) - a number followed by either cm or in:
    // If cm, the number must be at least 150 and at most 193.
    // If in, the number must be at least 59 and at most 76.

    var cmRegex = new RegExp(/^(\d+)cm$/, "g");
    var inRegex = new RegExp(/^(\d+)in$/, "g");

    var centimeters = cmRegex.exec(value)
    if(centimeters && centimeters.length == 2){
        var amount = Number(centimeters[1])
        return amount >= 150 && amount <= 193
    }

    var inches = inRegex.exec(value)
    if(inches && inches.length == 2){
        var amount = Number(inches[1])
...
function validateCid(passport, value){
